...
const getUserStats = async (userId) => {
    try {
        const balanceResult = await postgres_1.default.query("SELECT balance, bonus_balance, locked_balance FROM user_balances WHERE user_id = $1", [userId]);
        const betsResult = await postgres_1.default.query("SELECT COUNT(*) as total_bets, SUM(bet_amount) as total_wagered FROM bets WHERE user_id = $1", [userId]);
        const winsResult = await postgres_1.default.query("SELECT COUNT(*) as total_wins, SUM(win_amount) as total_won FROM bets WHERE user_id = $1 AND outcome = 'win'", [userId]);
        const favoritesResult = await postgres_1.default.query("SELECT COUNT(*) as favorite_games FROM user_game_preferences WHERE user_id = $1 AND is_favorite = true", [userId]);
        const levelResult = await postgres_1.default.query("SELECT ul.name as level_name, ulp.current_points, ulp.total_points_earned FROM user_level_progress ulp JOIN user_levels ul ON ulp.level_id = ul.id WHERE ulp.user_id = $1", [userId]);
